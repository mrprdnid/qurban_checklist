    ...
    public function index(Request $request)
    {
        ...
        $hewan = Hewan::with('checklistSembelih')
            ...
            ->when($sort,
                ...
                fn($q) => $q->orderByRaw('CASE WHEN checklist_sembelih.foto_sembelih = 1 AND checklist_sembelih.video_sembelih = 1 AND checklist_sembelih.otw_seset = 1 THEN 1 ELSE 0 END ASC')->orderBy('hewan.id', 'desc')
            )
            ->paginate(20)
            ->withQueryString();

        return view('checklist.sembelih.index', compact('hewan', 'q', 'status', 'sort', 'dir'));
    }
